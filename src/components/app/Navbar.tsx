...
export default function Navbar() {
    ...
    return (
        <nav className="z-20 flex-shrink-0 w-screen bg-white flex justify-between p-5 border-t border-gray-200">
            <a href="/dashboard" onClick={() => setLocation('dashboard')} className={`text-xl ${ location == 'notifications' ? 'text-amber-400' : 'text-gray-500'}`}>
                <Icon icon="fa6-solid:bell" className="w-4 h-4"/>
            </a>
            <a href="/dashboard" onClick={() => setLocation('dashboard')} className={`text-xl ${ location == 'tables' ? 'text-amber-400' : 'text-gray-500'}`}>
